...
static unsigned int __attribute__((aligned(16))) list[26214];
...
Image* loadImage(const char * filename, unsigned offset)
{
	png_structp png_ptr;
	png_infop info_ptr;
...
void initGraphics()
{
	...
	sceGumPerspective(75.0f, 16.0f/9.0f, 2.8f, 2000.0f); //Aplicar la perspectiva (angulo 75Âº, formato 16/9, 1.8 de cerca, 1000 de lejos)

    sceGumMatrixMode(GU_VIEW); //Iniciar modo de vision
	sceGumLoadIdentity();

	sceGuClearColor( GU_COLOR(0.0f, 1.0f, 1.0f, 1.0f ) );  //Limpiar pantalla con negro
	sceGuClearDepth(0); //Limpiar la profundidad
