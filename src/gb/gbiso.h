...
class gbiso
{
	...
	int extract(const char * file, const char * destination);
	...
	virtual int read(void * destination, unsigned size, unsigned count, void * p);
	virtual int seek(void * p, unsigned bytes, int flag);
