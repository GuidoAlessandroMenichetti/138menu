...
{
	char eboot_names[][13] = {"wmenu.bin\0", "EBOOT.PBP\0", "FBOOT.PBP\0", "VBOOT.PBP\0"};
    char * aux = NULL;
	int i;
...
int gbZip :: unzip(const char * file, const char * root, const char * eboot_name)
{
	...
	//memset(save_path, 0, ZIP_ENTRY_NAME+100);
        ...
        else
        {
			...
			*(entry_name + fileHeader.filenameLen) = '\0';
			...
			if(*(entry_name+strlen(entry_name)-1)=='/') //Is it a folder?
			{
				...
				/*if(gbExplorer::DirExist(save_path, DONT_CREATE)!=EXPLORER_OK)
				{
					if(gbExplorer::CreateDir(save_path)!=EXPLORER_OK) 
					{
						fclose(a);
						return ZIP_ERROR;
					};
